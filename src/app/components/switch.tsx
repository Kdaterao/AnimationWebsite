    ...
    return(
        ...
        <div className ='h-8 w-20 relative  rounded-full outline-[var(--color-navbar-t)] outline-6 '>
            <label className ={`h-full w-full relative  rounded-full flex flex-none bg-none outline-[var(--color-navbar-t)]  ${Darkstate ? '' : ''}`}>
                ...
                <span className={`h-8 w-12  rounded-full outline-3 outline-[var(--color-outline)] justify-self-start bg-[var(--color-navbar)] transition duration-75 ease-in-out flex ${Darkstate ? '-translate-x-1' : '-translate-x-12'}`}></span>
            </label>
        </div>
