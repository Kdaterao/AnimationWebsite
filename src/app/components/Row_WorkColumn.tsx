...
export default function Row_WorkColumn({title, description, imagepath, videokey, child_changevideo}:{title:string, description:string, imagepath:string, videokey:string, child_changevideo:(videokey:string,title:string, description:string) => void}){
        return(
        <div  onClick = {() => child_changevideo(videokey, title, description)} className ='flex flex-row gap-2 rounded-xl  bg-[var(--color-variablepurple)] shadow-md transition delay-100 duration-75 ease-in-out hover:-translate-y-2 hover:bg-[var(--color-variableblue)] h-20 lg:h-25 w-99/100'>
